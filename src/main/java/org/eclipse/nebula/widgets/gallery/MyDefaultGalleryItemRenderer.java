 ...
 *    
 *    Modifications: Sebastian Colutto
 *******************************************************************************/
 ...
 */
public class MyDefaultGalleryItemRenderer extends AbstractGalleryItemRenderer {
	private static final Logger logger = LoggerFactory.getLogger(MyDefaultGalleryItemRenderer.class);
	...
	public MyDefaultGalleryItemRenderer() {
		// Set defaults
	...
	public void draw(GC gc, GalleryItem item, int index, int x, int y,
		...
		if (item.getText() != null && !EMPTY_STRING.equals(item.getText())
				&& this.showLabels) {
			//new
			textHeight = gc.textExtent(item.getText()).y;
			fontHeight = gc.getFontMetrics().getHeight();
			//use text height instead of fontheight
			useableHeight -= textHeight + 2;
		}
		...
		if (_drawImage != null) {
			...
			if (item.getData("doNotScaleImage")==null) {
				...
				size = new Point(imageWidth, imageHeight);
...
//			logger.trace("size is: "+size);
		...
		if (_drawBackground) {
			...
			if (item.getText() != null && !EMPTY_STRING.equals(item.getText())
					&& showLabels) {
				//textheight instead of fontHeight
				gc.fillRoundRectangle(x, y + height - textHeight, width,
						textHeight, selectionRadius, selectionRadius);
			}
		}
