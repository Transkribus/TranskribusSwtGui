...
public class MyTableCombo extends Composite {
	 ...
	 * @see Widget#getStyle()
	 */
	public MyTableCombo(Composite parent, int style) {
	...
	private void createListeners() {
		...
		focusFilter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == MyTableCombo.this.getShell()) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (MyTableCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
...
//	}
	...
	private Label getAssociatedLabel() {
		...
		for (int i = 0; i < siblings.length; i++) {
			if (siblings[i] == MyTableCombo.this) {
...
//				Point size = arrow.getSize();				
		...
		case SWT.Help:
			...
			Composite comp = MyTableCombo.this;
			do {
