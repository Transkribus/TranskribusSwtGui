...
//				logger.debug("TargetControlListener: "+e.character + " keycode = "+e.keyCode + " traverse: "+e.type);
				...
				case SWT.LF:
				case SWT.CR:
//					logger.debug("enter pressed in autocomplete "+e.type);
					e.doit = false;
		...
		protected void adjustBounds() {
			...
			if (getProposalAcceptanceStyle() == PROPOSAL_INSERT || getProposalAcceptanceStyle() == PROPOSAL_REPLACE) {
	...
	private boolean autoActivateOnLetter;
	private boolean autoCloseOnSpace;
	...
	public ContentProposalPopup getPopup() {
		return popup;
	}
	 ...
	 */
	public void addContentProposalListener(MyIContentProposalListener2 listener) {
	 ...
	 * @see org.eclipse.jface.fieldassist.IContentProposalListener2
	 */
	public void removeContentProposalListener(MyIContentProposalListener2 listener) {
...
//								+" autoactivateonletter: "+isAutoActivateOnLetter());
	...
	private void notifyPopupOpened() {
		...
		for (int i = 0; i < listenerArray.length; i++) {
			((MyIContentProposalListener2) listenerArray[i]).proposalPopupOpened(this);
		}
	}
	...
	private void notifyPopupClosed() {
		...
		final Object[] listenerArray = proposalListeners2.getListeners();
		for (int i = 0; i < listenerArray.length; i++) {
			((MyIContentProposalListener2) listenerArray[i]).proposalPopupClosed(this);
		}
	}
	...
	public boolean isAutoActivateOnLetter() {
		return autoActivateOnLetter;
	}

	public void setAutoActivateOnLetter(boolean autoactivateOnLetter) {
		this.autoActivateOnLetter = autoactivateOnLetter;
	}

	public boolean isAutoCloseOnSpace() {
		return autoCloseOnSpace;
	}

	public void setAutoCloseOnSpace(boolean autoCloseOnSpace) {
		this.autoCloseOnSpace = autoCloseOnSpace;
	}
