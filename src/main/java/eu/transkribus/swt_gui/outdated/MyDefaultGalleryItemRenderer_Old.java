 ...
 */
@Deprecated
public class MyDefaultGalleryItemRenderer_Old extends AbstractGalleryItemRenderer {
	private final static Logger logger = LoggerFactory.getLogger(MyDefaultGalleryItemRenderer_Old.class);
	...
	public MyDefaultGalleryItemRenderer_Old() {
	...
	public void draw(GC gc, GalleryItem item, int index, int x, int y, int width, int height) {
		...
		if (item.getText() != null && showLabels) {
			...
			int drawy = y + height - fontHeight;
			for (String line : text.split("\n")) {
				logger.debug("drawing line: "+line);
				gc.drawText(line, x + textxShift, drawy+10, true);
				drawy += fontHeight;
			}
