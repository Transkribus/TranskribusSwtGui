...
//				logger.error(e.getMessage(), e);
	...
	public ThumbnailWidgetVirtual(Composite parent, int style) {
		super(parent, style);
...
//		gallery = new Gallery(groupComposite, SWT.V_SCROLL | SWT.SINGLE);
		gallery = new Gallery(groupComposite, SWT.V_SCROLL | SWT.SINGLE | SWT.VIRTUAL);
...
//						infoTi.setWidth(100);
		...
		gallery.addListener(SWT.SetData, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				int index;
				if (item.getParentItem() != null) { // if this is a leaft item -> set nr of items to 0! 
					index = item.getParentItem().indexOf(item);
					item.setItemCount(0);
				} else {
					index = gallery.indexOf(item);
					item.setItemCount(urls.size());
				}
...
//				
//				
//				item.setText("aklajdlkfjalsdfjlkasjd");
//				
				setGalleryItemData(item, index);
			}
		});	
	...
	protected void showPageManager() {
		...
		if(isManagerOpen(tm)){
			...
			tm = new ThumbnailManagerVirtual(getShell(), SWT.NONE, this);
			tm.open();
		}
...
//			reload();
	...
	private void createGalleryItems(){
		...
		logger.debug("createGalleryItems, group.getItemCount() = "+group.getItemCount());
		...
		for (int i=0; i<urls.size(); ++i) {
			...
			setGalleryItemData(item, i);			
		}
	}
	
	private void setGalleryItemData(GalleryItem item, int index) {
...
//		item.setText(1, "String 1");
		...
		String transcribedLinesText = determineItemColor(item, transcripts.get(index));

		setItemText(item, index, transcribedLinesText);
	}
