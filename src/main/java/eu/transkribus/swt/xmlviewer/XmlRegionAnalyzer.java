...
import eu.transkribus.swt.xmlviewer.XmlRegion.XmlRegionType;
...
public class XmlRegionAnalyzer {
		...
		while (this.offset < xml.length()) {
			...
			if (c == '<') {
				...
				if (analyzeCData(xml, positions))
					continue;
				positions.add(new XmlRegion(XmlRegionType.UNEXPECTED,
			...
			if (analyzeMarkupValue(xml, positions))
				continue;
			positions.add(new XmlRegion(XmlRegionType.UNEXPECTED, this.offset,
					xml.length()));
			break;
		}
		return positions;
	...
	boolean analyzeInstruction(String xml, List<XmlRegion> positions) {
		...
		if (newPos < xml.length() && xml.charAt(newPos) == '<'
				&& ++newPos < xml.length() && xml.charAt(newPos) == '?') {
			while (++newPos < xml.length() && xml.charAt(newPos) != '>')
				newPos = xml.indexOf('?', newPos);
			if (xml.charAt(newPos) == '>') {
				positions.add(new XmlRegion(XmlRegionType.INSTRUCTION,
	...
	boolean analyzeComment(String xml, List<XmlRegion> positions) {
		boolean result = false;
		int newPos = this.offset;
		if (xml.charAt(newPos) == '<' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '!' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '-' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '-') {
			int seq = 0;
			...
			if (seq == 3)
				newPos++;
			positions.add(new XmlRegion(XmlRegionType.COMMENT, this.offset,
					newPos));
			this.offset = newPos;
			result = true;
		}
	...
	boolean analyzeMarkup(String xml, List<XmlRegion> positions) {
		...
		if (xml.charAt(newPos) == '<') {
			...
			if (c == '>')
				newPos++;
			positions.add(new XmlRegion(XmlRegionType.MARKUP, this.offset,
					newPos));
			this.offset = newPos;
			result = true;
		}
		// "/>"
		else if (xml.charAt(newPos) == '/' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '>') {
			positions.add(new XmlRegion(XmlRegionType.MARKUP, this.offset,
					++newPos));
			this.offset = newPos;
	...
	boolean analyzeAttribute(String xml, List<XmlRegion> positions) {
		...
		if (newPos != this.offset) {
			positions.add(new XmlRegion(XmlRegionType.ATTRIBUTE, this.offset,
					newPos));
			this.offset = newPos;
			result = true;
		}
	...
	boolean analyzeMarkupValue(String xml, List<XmlRegion> positions) {
		...
		if (this.offset != newPos) {
			...
			positions.add(new XmlRegion(XmlRegionType.MARKUP_VALUE, start,
					newPos));
			this.offset = newPos;
			result = true;
		}
	...
	boolean analyzeAttributeValue(String xml, List<XmlRegion> positions) {
		...
		if (xml.charAt(newPos) == '=') {
			...
			positions.add(new XmlRegion(XmlRegionType.ATTRIBUTE_VALUE,
					this.offset, newPos));
			this.offset = newPos;
			result = true;
		}
	...
	boolean analyzeCData(String xml, List<XmlRegion> positions) {
		...
		if (xml.charAt(newPos) == '<' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '!' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '[' && ++newPos < xml.length()
				&& xml.charAt(newPos) == 'C' && ++newPos < xml.length()
				&& xml.charAt(newPos) == 'D' && ++newPos < xml.length()
				&& xml.charAt(newPos) == 'A' && ++newPos < xml.length()
				&& xml.charAt(newPos) == 'T' && ++newPos < xml.length()
				&& xml.charAt(newPos) == 'A' && ++newPos < xml.length()
				&& xml.charAt(newPos) == '[') {
			int cpt = 0;
			while (++newPos < xml.length()) {
			...
			positions.add(new XmlRegion(XmlRegionType.CDATA, this.offset,
					newPos));
			this.offset = newPos;
			result = true;
		}
	 ...
	 * <p>
	 * If white spaces are found, a XML position is stored and the offset is
	 * updated.
	 * </p>
	 * 
	 * @param xml
	...
	void analyzeWhitespaces(String xml, List<XmlRegion> positions) {
		...
		while (i < xml.length() && Character.isWhitespace(xml.charAt(i)))
			i++;
		if (i != this.offset) {
			positions.add(new XmlRegion(XmlRegionType.WHITESPACE, this.offset,
					i));
			this.offset = i;
		}
	}
